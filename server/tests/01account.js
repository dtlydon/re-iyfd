...
describe('accounts', () => {
  ...
  it('should add users', async () => {
    ...
    const response1 = await axios.post(`${baseUrl}/account/add-users`, {
      users: ['dan da man 1', 'dan da man 2', 'dan da man 3']
    }, {
      headers: {
        token
      }
  ...
  it('should fail, not an admin', async () => {
    try {
      await axios.get(`${baseUrl}/account/all`, {
        headers: {
          token: getToken()
        }
      });
  ...
  it('should creates second account', async () => {
    ...
    assert.equal(response.status, 200);
    const response1 = await axios.post(`${baseUrl}/account/add-user`, {
      user: 'Test user 1'
    }, {
      headers: {

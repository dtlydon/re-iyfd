...
const pickWinnersPerRound = async (round) => {
  const response = await axios.get(`${baseUrl}/matchups`, {
    headers: {
      token: getToken()
    }
  });
  ...
  for (const matchUp of matchUps.filter(m => m.round === round)) {
    ...
    await axios.put(`${baseUrl}/matchups/${matchUp._id}`, {
      winner: matchUp.entry1._id
    }, {
      headers: {
        token: getToken()
      }
  ...
  const response2 = await axios.get(`${baseUrl}/matchups`, {
    headers: {
      token: getToken()
    }
  });
...
const simUserPicksPerRound = async (round) => {
  ...
  const promises = userChoices.filter(x => x.matchUp.round === round).map(async (userChoice) => {
    const choice = pickFirstEntry ? userChoice.matchUp.entry1._id : userChoice.matchUp.entry2._id;
    pickFirstEntry = !pickFirstEntry;
    return axios.put(`${baseUrl}/choices/${testUserId}`, {
      choice,
      matchUpId: userChoice.matchUp._id
    }, getBasicHeaders());
  });
...
describe('SimulateTourny', () => {
  ...
  it('should prevent picks after winner is selected', async () => {
    ...
    assert.equal(userChoice.choice, userChoice.matchUp.entry1._id);
    const choiceId = userChoice._id;
    try {
      await axios.put(`${baseUrl}/choices/${testUserId}`, {
        choice: userChoice.matchUp.entry2._id,
        matchUpId: userChoice.matchUp._id
      }, getBasicHeaders());
      assert(false);
    } catch (err) {
